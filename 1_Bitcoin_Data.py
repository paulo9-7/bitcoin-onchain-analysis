...
end_date = datetime.date(2020, 11, 9)
...
for single_date in tqdm(date_range):
    ...
    file_name = (
        f"blockchair_bitcoin_blocks_{file_date}.tsv.gz"  # Construct the file name
    )
...
source_dir = BASE / "gz_files/"

# Directory to save the decompressed .tsv files
output_dir = BASE / "tsv_files/"
...
for filename in os.listdir(source_dir):
    if filename.endswith(".gz"):
        ...
        tsv_path = os.path.join(
            output_dir, filename[:-3]
        )  # Removes the last 3 characters ('.gz')
...
combined_df.to_csv(BASE / "tsv_files/Bitcoin_Combined_20191111_20201109.tsv", sep="\t", index=False)
